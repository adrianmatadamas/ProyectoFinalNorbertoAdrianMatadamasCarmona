...
function mostrarResultado() {
    ...
    resultadoElemento.classList.add('animate__fadeIn');
...
// Función para cargar datos desde un JSON local y mostrarlos
async function cargarEstadisticas() {
    try {
        const response = await fetch('estadisticas.json');
        const data = await response.json();
        const estadisticasElemento = document.getElementById('estadisticas');
        estadisticasElemento.innerHTML = `
            <p>Total de operaciones: ${data.totalOperaciones}</p>
            <p>Operación más común: ${data.operacionMasComun}</p>
            <p>Promedio de resultados: ${data.promedioResultados}</p>
        `;
    } catch (error) {
...
// Función para obtener el clima actual utilizando wttr.in
async function obtenerClima(ciudad = 'Mexico City') {
    try {
        ...
        climaElemento.innerHTML = `
            <p>Ciudad: ${ciudad}</p>
            <p>Clima: ${clima.weatherDesc[0].value}</p>
        ...
        const climaElemento = document.getElementById('clima');
        climaElemento.innerHTML = `<p>Error al obtener el clima</p>`;
    }
}
